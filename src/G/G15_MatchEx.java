...
public class G15_MatchEx {
	...
	/*
	 * 	스트림 내부에 요소들이 특정 조건을 만족하였는지 검사할 수 있는 메서드
	 * 	allMatch (Predicate p) - 요소들이 모두 p조건을 만족하면 ture, 그렇지 않으면 false
	 * 	anyMatch (Predicate p) - 요소들 중 하나라도 p조건을 만족하면 true, 그렇지 않으면 false
	 * 	noneMatch (Predicate p) - 요소들이 모두 p조건을 만족하지 않으면 ,true 그렇지 않으면 false
	 */
