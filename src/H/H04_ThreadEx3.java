...
public class H04_ThreadEx3 {
	 ...
	 * 	 하나의 스레드가 진행중인 작업이 끝날때까지 객체가 변경되지 않도록 하는 것을 동기화(synchronized) 라고 한다.
	 * 
...
//	public synchronized void increaseLevel() {
	public void increaseLevel() {	
		...
		while (true) {
			...
			if (this.level == 5 ) {
				notifyAll();		// 현재 대기중인 모든 스레드를 실행 상태로 전환한다.
				try {
					// notify(); 	// 현재 대기중인 스레드를 한개만 실행 중으로 변환( 직접 지정 불가하다 )
					wait();			// 현재 실행 중인 스레드를 대기 상태로 전환한
				} catch (InterruptedException e) {}
			}
