...
public class G09_StreamEx2 {
	...
	/*
	 *		Stream 객체의 최상위는 BaseStream 인터페이스 이고.
	 *			{Stream(Object), IntStream(int), LongStream(long), DoubleStream(double)} 을 가지고 있다.
	 *		각 자료 타입에 따라서 Stream 을 구분하여 사용할 수 있다.
	 */
	public static void main(String[] args) {
		...
		System.out.println("IntStream.of() 로 IntStream 생성하기");
		IntStream intstream2 = IntStream.of(arr);
		intstream2.forEach(s -> System.out.print(s + "\t"));
		System.out.println();
		
		System.out.println("IntStream.range(1, 6) IntStream 생성하기");		// 끝 값을 포함하지 않는다
		...
		IntStream isr = new Random().ints(3);		// ints(3) 은 3개의 정수 난수 생성 을 의미한다.
		...
		DoubleStream dsr = new Random().doubles(4);
		dsr.forEach(s -> System.out.print(s + " "));
		System.out.println();
		
		// 문자열을 Stream 으로 처리하기
		// 	chars() 메서드의 사용
		String str = "자바 세상을 만들어보자";
		// Stream 객체 생성하기
		isr = str.chars();	// why? IntStream을 사용했을까?	char는 정수 계산이 가능하기 때문에.
		
		isr.forEach(s -> System.out.print((char)s));			// (char)캐스팅 하여 문자로 출력
		System.out.println();
